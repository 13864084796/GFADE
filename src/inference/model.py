...
class MixStyle(nn.Module):
    def __init__(self, p=0.0000001, alpha=0.1):
...
#         # self.dropout = nn.Dropout(p=0.5)  # Adding Dropout with 50% dropout rate
#         self.cel = nn.CrossEntropyLoss()
#         self.label_smoothing_loss = LabelSmoothingLoss(classes=2, smoothing=0.1)
#         self.optimizer = SAM(self.parameters(), torch.optim.SGD, lr=0.001, momentum=0.9)
#         # # --------------------------asam优化器----------------------------------#
#         # self.optimizer = torch.optim.SGD(self.parameters(),lr=0.001,momentum=0.9,weight_decay=1e-4)
#         # self.minimizer = ASAM(self.optimizer,self,rho=0.4,eta=0.0)
#         # # -----END-----#
...
#             transforms.RandomRotation(degrees=15)  # 可调整旋转的角度范围
#         ])
#
#     def forward(self, x):
#         x = self.net(x)
